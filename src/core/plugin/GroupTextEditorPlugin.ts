...
class GroupTextEditorPlugin {
  ...
  _init() {
    ...
    this.canvas.on('mouse:up', () => {
      const textBox: any = this.canvas.getActiveObject();
      if (textBox && textBox.get('type') === 'textbox' && textBox.scaleY && textBox.scaleY !== 1) {
        const newWidth = textBox.width * textBox.scaleX;
        const newHeight = textBox.height * textBox.scaleY;
        textBox.set({ width: newWidth, height: newHeight, scaleX: 1, scaleY: 1 });
      }
      this.isDown = false;
    });
