...
import { fitImg } from '@/hooks/fitImg';
...
function intervalControl() {
  ...
  fabric.Object.prototype.controls.ml = new fabric.Control({
    ...
    mouseUpHandler: (_, transform) => mouseCtrl(_, transform, 'horizontal'),
  ...
  fabric.Object.prototype.controls.mb = new fabric.Control({
    ...
    mouseUpHandler: (_, transform) => mouseCtrl(_, transform, 'vertival'),
    getActionName: fabric.controlsUtils.scaleOrSkewActionName,
  ...
  fabric.Object.prototype.controls.mt = new fabric.Control({
    ...
    cursorStyleHandler: fabric.controlsUtils.scaleSkewCursorStyleHandler,
    actionHandler: fabric.controlsUtils.scalingYOrSkewingX,
    mouseUpHandler: (_, transform) => mouseCtrl(_, transform, 'vertical'),
    getActionName: fabric.controlsUtils.scaleOrSkewActionName,
    render: renderIconHoz,
  });
}
...
function mouseCtrl(_: any, transform: any, direction: string) {
    ...
    case 'image':
      target.set({
        clipPath: new fabric.Rect({
          left: -((target.width || 0) / 2),
          top: -((target.height || 0) / 2),
          width: target.width,
          height: target.height,
          rx: target.rx / (target.scaleX || 1),
          ry: target.ry / (target.scaleY || 1),
        }),
      });
      break;
    case 'circle':
      const newRadius = direction === 'horizontal' ? (target.width || 0) * (target.scaleX || 0) / 2 : (target.height || 0) * (target.scaleY || 0) / 2;
      target.set({ radius: newRadius, scaleX: 1, scaleY: 1, width: newRadius * 2, height: newRadius * 2 });
      break;
    case 'rect':
      newWidth = (target.width || 0) * (target.scaleX || 0);
      newHeight = (target.height || 0) * (target.scaleY || 0);
      target.set({ width: newWidth, height: newHeight, scaleX: 1, scaleY: 1 });
      if (target.imageType) {
        globalEventEmitter.emit('image:resized');
      }
      break;
    default:
      newWidth = (target.width || 0) * (target.scaleX || 0);
  ...
  globalEventEmitter.emit('object:modified');
  return true;
}
...
class ControlsPlugin {
  ...
  constructor(canvas: fabric.Canvas, editor: IEditor) {
    this.canvas = canvas;
    this.editor = editor;
    this.debouncePreview = debounce(this.editor.previewPoster, 100);
    this.init();
  }
  init() {
    // 删除图标
    ...
    const _this = this;
    globalEventEmitter.on('object:modified', this.debouncePreview)
    this.editor.canvas.on('object:moving', () => {_this.isMoving = true})
    this.editor.canvas.on('object:modified', () => {if (_this.isMoving) {_this.editor.previewPoster(); _this.isMoving = false;}})
    // textbox保持一致
