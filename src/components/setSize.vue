    ...
    >
      ...
      <Form :label-width="40" class="form-wrap" style="justify-content: flex-start">
        <FormItem :label="$t('width')" prop="name" style="margin-right: 10px">
          <InputNumber :min="1" :max="5000" v-model="width"></InputNumber>
        </FormItem>
        <FormItem :label="$t('height')" prop="name">
          <InputNumber :min="1" :max="5000" v-model="height"></InputNumber>
        </FormItem>
      </Form>
    </Modal>
  </div>
...
onMounted(() => {
  ...
  const poster = canvasEditor.canvas.getObjects()[1]
  poster.set({
    width: width.value,
    height: height.value,
  })
